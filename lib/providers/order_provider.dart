...
Future<void> loadStatistics() async {
  ...
  void filterByStatus(String? status) {
    _selectedStatuses.clear(); // Clear all previous selections
    if (status != null) {
      _selectedStatuses.add(status); // Add only one status
    }
    _applyFilters();
    notifyListeners();
  }
  ...
  void toggleStatusFilter(String status) {
    if (_selectedStatuses.contains(status)) {
      _selectedStatuses.clear(); // Clear if same status clicked
    } else {
      _selectedStatuses.clear(); // Clear all
      _selectedStatuses.add(status); // Add only new status
    }
    _applyFilters();
    notifyListeners();
  }
